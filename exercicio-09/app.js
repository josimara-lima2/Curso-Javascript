...
*/
 ...
 const numberCaracteres = string =>  string.length
 
 const myString = 'Teste 2'
 console.log(`A string "${myString}" tem ${numberCaracteres(myString)} caracteres`)
/*
  03
...
console.log(indexCaractere('m', 'amor'))
/*
  05
...
*/
const exist = (item, array) => array.includes(item)
  
  

console.log(exist(1,[1,2,3]))
/*
  06
...
console.log(concatenaArray([1,2],[3,4]))
/*
  07
...
*/
const removeUltimo = array => {
  
   array.pop()
  return array
}

console.log(removeUltimo([1,2,3]))
/*
  08
...
*/
const callNumber = (callback, value) => {
...
console.log(callNumber(triple, 33))
...
numbers.forEach((i, index, array)=> {
  console.log(`O ${index + 1}º item do array [${array}] é ${i}.`)
})
/*
  12
...
letters.forEach(i => lettersCopy.push(i))
...
const curtidas = (arrayNames=[]) => {
  const ultimoElemento = arrayNames[arrayNames.length - 1]
  
  switch (arrayNames.length) {
    case 0:
     return 'Ninguém curtiu isso'
    case 1:
     return `${arrayNames} curtiu isso`
    case 2:
     return `${arrayNames.join(' e ')} curtiram isso`
    case 3:
     return `${arrayNames.join(', ').replace(`, ${ultimoElemento}`, ` e ${ultimoElemento}`)} curtiram isso`
    default:
     return `${arrayNames[0]}, ${arrayNames[1]} e mais ${arrayNames.length - 2} pessoas curtiram isso`
...
console.log(curtidas(['ana', 'maria', 'luana']))
console.log(curtidas(['ana']))
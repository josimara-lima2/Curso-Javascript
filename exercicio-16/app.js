...
const showClickedElement = ({target}) => {
  const clickedNameElement = target.tagName.toLowerCase()
  if(clickedNameElement === 'div'){
    h2.textContent = 'Clicou na div.'
    return
  }
  h2.textContent = `Clicou no ${clickedNameElement} filho da div.`
 }
...
const showMoveMessage = ({offsetX,offsetY}) =>{   
...
egg.addEventListener('mousemove', showMoveMessage)
...
const contemFrontEndDeveloper = people.some(({profession}) =>
     profession === 'Front-end developer')
if(contemFrontEndDeveloper){

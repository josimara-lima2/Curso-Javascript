...
// O erro acontece por a variável age tem escopo local, então so pode ser acessada dentro do bloco onde foi declarado.
...
const car = {
  ...
  getColorsMessage (){
    const ultimoElemento = this.colors[this.colors.length - 1]
    const colors = this.colors.join(', ').replace(`${ultimoElemento}`, ` e ${ultimoElemento}`)
    return `${this.name} está disponível nas cores ${colors}`
  }
}
/*
  04
